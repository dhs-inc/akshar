...
groupsRouter.post('/mesh/groups', requireAuth, async (req: Request, res: Response) => {
...
groupsRouter.get('/mesh/groups', requireAuth, async (req: Request, res: Response) => {
  const userId = (req as any).userId;
  const groups = await groupService.getUserGroups(userId);
  res.json({ ok: true, groups });
});

groupsRouter.get('/mesh/groups/:groupId', requireAuth, async (req: Request, res: Response) => {
  const userId = (req as any).userId;
