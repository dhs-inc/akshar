 ...
 */
export function wrapOnion(payload, hops) {
    ...
    let current = encrypt(hops[hops.length - 1].key, JSON.stringify({ inner: payload, next: null }));
    // Work outward: each layer wraps the previous, adding next-hop address
    for (let i = hops.length - 2; i >= 0; i--) {
        current = encrypt(hops[i].key, JSON.stringify({ inner: current, next: hops[i + 1].nextAddr }));
    }
    return {
        ciphertext: current,
 ...
 */
export function peelOnion(encryptedPayload, key) {
 ...
 */
export function wrapReturnOnion(payload, reverseHops) {
